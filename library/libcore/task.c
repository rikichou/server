...
void taskFreeAll(void)
{
    ...
    while(list)
    {
        ...
        while(task)
        {
            ...
            debug("task", " Remove task(%s-%s)", taskTargetString(&task->target), task->name);                
            free(task);
        ...
        debug("task", "Remove task list(%s)", taskTargetString(&list->target)); 
        free(list);
    ...
    while(list)
    {
        ...
        while(task)
        {
                ...
                || (taskStateFailed(task) && task->retry))
            {
                ...
                if (task->start)
                {
                    debug("task", "Task(%s-%s) start", taskTargetString(&task->target), task->name);
                    task->start(task);
                }
            }
            else if (taskStateDone(task))
            {
                ...
                debug("task", "Task(%s-%s) done, remove", taskTargetString(&task->target), task->name);  
        ...
        if (list->head == NULL)
        {
            ...
            debug("task", "Task list(%s) empty, remove", taskTargetString(&list->target)); 
...
int taskPush(struct task *task)
{
    ...
    if (list == NULL)
    {
        ...
        if (list == NULL)
        {
            debugf("malloc", "malloc(%d)", sizeof(struct taskList));
            free(task);            
            return 0;
        }
    ...
    else 
    {
        ...
        while(p)
        {
            ...
            {                
                debug("task", "Task(%s-%s) already queued", taskTargetString(&task->target), p->name);                
                free(task);
                return 0;
            }
            p = p->next;
    ...
    debug("task", "Push task : %s-%s", taskTargetString(&task->target), task->name);
